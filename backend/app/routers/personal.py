...
from app.token_limit_handler import token_required, create_token_limit_response, TokenLimitError
import uuid
...
def send_personal_message(message: ChatMessage, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        # Re-lançar HTTPException (como token limits)
        raise e
    except Exception as e:
        print(f"[PERSONAL] ❌ Erro inesperado: {e}")
        raise HTTPException(
